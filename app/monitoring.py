...
# === JSON Formatter & Logger Setup ===


class JSONFormatter(logging.Formatter):
    """Format log records as JSON for log aggregation (ELK, Datadog, etc.)."""

    def format(self, record): #record is automatically passed , it contains metadata
        log_obj = {
        ...
        if hasattr(record, "extra_data"): #hasattr(object, name),checks if the object has an attribute (or method) with that name and returns True or False
...
class MetricsCollector:
    ...
    def summary(self) -> dict:
        ...
        cache_hit_rate = (
            (self._cache_hits / total_cache) * 100 if total_cache > 0 else 0.0
        )
